...
describe('assetclass builder module', () => {
  test('test assetclass ada builder', () => {
    ...
    expect(ada.assetId).toBe(ADA_TOKEN_NAME);
  });
  ...
  test('test assetclass native asset builder', () => {
    ...
    const min = AssetClassBuilder.new().currencySymbol(minswap_cs).assetId(minswap_tkn).build();
    expect(min).toBeTruthy();
    expect(min.currencySymbol).toBe(minswap_cs);
    expect(min.assetId).toBe(minswap_tkn);
  });

  test('test assetclass native asset builder with invalid currency symbol', () => {
    const cs = 'abc';
    try {
      const min = AssetClassBuilder.new().currencySymbol(cs).assetId('4d494e').build();
